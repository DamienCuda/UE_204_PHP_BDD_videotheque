        ...
        <body>
            ...
            <main>
                ...
                if (count($user_datas) != 0) {
                    ...
                    $rented_movies_req = $conn->prepare('SELECT DISTINCT catalogue.id, catalogue.movie_picture, catalogue.release_year, catalogue.title, catalogue.director FROM catalogue AS catalogue JOIN movies_location AS rented_movies ON catalogue.id = rented_movies.movie_id && rented_movies.user_id = ? LIMIT 4');
                    $rented_movies_req->execute(array(nettoyage($_GET['id'])));
                    $user_rented_list = $rented_movies_req->fetchAll(PDO::FETCH_ASSOC);

                    //Requête pour compter les films loués par l'utilisateur
                    $count_movies_loc = $conn->prepare("SELECT count(*) as total FROM movies_location WHERE user_id = ? AND is_loc = ?");
                    $count_movies_loc->execute([nettoyage($_GET['id']), 1]);
                    ...
                    $rented_movies_req = $conn->prepare('SELECT DISTINCT catalogue.id, catalogue.movie_picture, catalogue.release_year, catalogue.title, catalogue.director FROM catalogue AS catalogue JOIN movies_location AS rented_movies ON catalogue.id = rented_movies.movie_id && rented_movies.user_id = ? && rented_movies.is_loc = 1 LIMIT 4');
                    $rented_movies_req->execute(array(nettoyage($_GET['id'])));
                    $user_rented_list_now = $rented_movies_req->fetchAll(PDO::FETCH_ASSOC);

                    //Requête pour compter les films loués par l'utilisateur en ce moment
                    $count_movies_his = $conn->prepare("SELECT count(*) as total FROM movies_location WHERE user_id = ?");
                    $count_movies_his->execute([nettoyage($_GET['id'])]);
                    $result_his = $count_movies_his->fetch();
                    $total_movie_his = $result_his['total'];

                    ?>
                    <!-- BEGIN: Recherche d'utilisateurs -->
                    <section id="user_search" class="container">
                        <div class="row">
                            <?php if (isset($_GET['id']) && $_GET['id'] != $_SESSION['id']) { ?>
                                <button class="btn btn-light btn-back d-flex align-items-center justify-content-between mt-5"
                                        id="back"><i class='bx bx-left-arrow-alt'></i><span>Retour</span></button>
                            <?php } ?>
                        </div>
                        <div class="row <?php if (isset($_GET['id']) && $_GET['id'] != $_SESSION['id']) {
                            echo "mt-3";
                        } else {
                            echo "mt-5";
                        } ?>">
                            <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 text-center" id="container-search-user">
                                <input type="text" name="search_user" id="search_user" placeholder="Rechercher un utilisateur"
                    ...
                    <!-- END: Recherche d'utilisateurs -->

                    <!-- BEGIN: Infos utilisateur -->
                    <section id="user_infos_container" class="container">
                        <!-- BEGIN: Message de bienvenue -->
                        <div class="row mt-5">
                        ...
                        <!-- END: Message de bienvenue -->

                        <!-- BEGIN: Informations détails -->
                        <div class="row mb-5">
                            <div class="col-12 col-sm-12 col-md-12 col-lg-3 col-xl-3 text-center ">
                                <div id="profil_picture">
                                    <!-- BEGIN: Image du profil selon si l'utilisateur en a une ou non -->
                                    ...
                                    if ($user_profil_pic == null) {
                                        ...
                                        <img class="rounded-circle mb-3"
                                             src="users/<?= $_GET['id']; ?>/avatar/<?= $user_profil_pic; ?>" alt="Avatar"
                                             style="height:150px;width:150px;object-fit: cover;">
                                        <?php
                                    }
                                    ?>
                                </div>
                                <!-- END: Image du profil selon si l'utilisateur en a une ou non -->
                            </div>
                            <div class="col-12 col-sm-12 col-md-12 col-lg-9 col-xl-9 text-light text-center text-sm-center text-md-center text-lg-start text-xl-start">
                                ...
                                <p class="mb-2 d-flex align-items-center justify-content-center justify-content-sm-center justify-content-md-center justify-content-lg-start justify-content-xl-start">
                                    <a href="liste.php?id=<?= $_GET['id']; ?>" class="text-light"><i
                                                class='bx bx-plus mr-2'></i><span><?php if ($_GET['id'] == $_SESSION['id']) {
                                                echo "Ma liste";
                                            } else {
                                                echo "Sa liste";
                                            } ?></span></a></p>
                                <?php if (isset($_GET['id']) && $_GET['id'] == $_SESSION['id']) { ?>
                                    <p class="mb-2 d-flex align-items-center justify-content-center justify-content-sm-center justify-content-md-center justify-content-lg-start justify-content-xl-start">
                                        <a href="user_transaction.php?user_id=<?= $_GET['id']; ?>" class="text-light"><i
                                                    class='bx bx-euro'></i><span>Mes transactions</span></a></p>
                                <?php } ?>
                                <?php
                                if (isset($_GET['id']) && $_GET['id'] == $_SESSION['id']) {
                                    ?>
                                    <!-- BEGIN: Boutons de modifications du profil -->
                                    <div class="d-flex mt-4 justify-content-center justify-content-sm-center justify-content-md-center justify-content-lg-start justify-content-xl-start">
                                        ...
                                        <button class="btn btn-light d-none" id="btn_edit_profil_skip">Annuler les
                                            modifications
                                        </button>
                        ...
                        <!-- END: Informations détails -->
                    </section>
                    <!-- END: Infos utilisateur -->

                    <?php if (isset($_GET['id']) && $_GET['id'] == $_SESSION['id']) { ?>
                        <!-- BEGIN: Location en cours -->
                        <section id="current_locations" class="container">
                            <div class="row mb-5">
                                <!-- BEGIN: header location en cours -->
                                <h3 class="p-2 rounded yellow-bg d-flex align-items-center"><span class="mr-2">Locations en cours</span><i
                                            class='bx bx-cart-download'></i></h3>
                                <!-- END: header location en cours -->
                                <div class="row mt-5">
                                    <?php
                                            ...
                                            <div class="col-12 col-sm-12 col-md-6 col-lg-4 col-xl-3 d-flex justify-content-center justify-content-sm-center justify-content-md-start justify-content-lg-start justify-content-xl-start mb-4">
                                                ...
                                                <div class="card movie">
                                                    ...
                                                    if ($diff < 0) {
                                                        ?>
                                                        <!-- BEGIN: countdown -->
                                                        <div id="countdown_<?= $movie_isloc['id'] ?>"
                                                             class="countdown d-flex align-items-center">
                                                            <div class="days"></div>
                                ...
                                <?php if ($total_movie_loc > 4) { ?>
                                    <a href="locations-list.php?id=<?= $_GET['id']; ?>">
                                        <div class="d-flex justify-content-end">
                                            <button class="btn btn-warning">Voir tout</button>
                                        </div>
                                    </a>
                                <?php } ?>
                            </div>
                        </section>
                        <!-- END: Location en cours -->

                        <!-- BEGIN: historique des locations -->
                        <section id="past_locations" class="container">
                            <!-- BEGIN: Header historique des locations -->
                            <h3 class="p-2 rounded yellow-bg d-flex align-items-center"><span class="mr-2">Historique des locations</span><i
                                        class='bx bx-history'></i></h3>
                            <!-- END: Header historique des locations -->
                            <div class="row mt-5">
                                <?php
                                ...
                                <?php if ($total_movie_his > 4) { ?>
                                    <a href="locations-historique.php?id=<?= $_GET['id']; ?>">
                                        <div class="d-flex justify-content-end">
                                            <button class="btn btn-warning">Voir tout</button>
                                        </div>
                                    </a>
