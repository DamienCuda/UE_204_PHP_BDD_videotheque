        ...
        <body>
            ...
            <main>
                ...
                if (count($user_datas) != 0) {
                    ...
                    $rented_movies_req_count = $conn->prepare('SELECT COUNT(*) AS nb_movies FROM catalogue AS catalogue JOIN movies_location AS rented_movies ON catalogue.id = rented_movies.movie_id && rented_movies.user_id = ?');
                    ...
                    $rented_movies_req = $conn->prepare('SELECT DISTINCT catalogue.id, catalogue.movie_picture, catalogue.release_year, catalogue.title, catalogue.director FROM catalogue AS catalogue JOIN movies_location AS rented_movies ON catalogue.id = rented_movies.movie_id && rented_movies.user_id = :id ORDER BY id DESC LIMIT :premier, :parpage;');
